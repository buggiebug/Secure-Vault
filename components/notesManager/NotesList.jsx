...
import { deleteNote, syncTodosThunk } from '../../redux/slice/todoSlice';

const NotesList = ({ onOpen, onCreate }) => {
    const tasks = useSelector(state => state.todo.tasks);
    const syncStatus = useSelector(state => state.todo.syncStatus);
    const dispatch = useDispatch();

    // Sort notes purely by last modified time (recently edited first)
    const sortedNotes = useMemo(() => {
        return Object.values(tasks).sort((a, b) => {
            const bTime = new Date(b.updatedAt || b.createdAt || b.date).getTime() || 0;
            const aTime = new Date(a.updatedAt || a.createdAt || a.date).getTime() || 0;
            return bTime - aTime;
        });
    }, [tasks]);
            ...
            <FlatList
                ...
                refreshControl={
                    <RefreshControl
                        ...
                        onRefresh={() => dispatch(syncTodosThunk())}
                        colors={['#6C63FF']}
                        tintColor="#6C63FF"
                    />
                }
